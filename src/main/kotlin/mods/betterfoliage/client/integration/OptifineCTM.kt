...
object OptifineCTM {
    ...
    val isAvailable =
        allAvailable(
            Refs.ConnectedTextures,
            Refs.ConnectedProperties,
            Refs.getConnectedTexture,
            Refs.CTblockProperties,
            Refs.CTtileProperties,
            Refs.CPtileIcons,
            Refs.CPmatchBlocks,
            Refs.CPmatchTileIcons,
        )
    ...
    fun Any.matchesBlock(block: Block) = (Refs.CPmatchBlocks.get(this) as IntArray?)?.let { Block.getIdFromBlock(block) in (it) }
        ?: false

    /** Does this ConnectedProperties instance match the given texture? */
    fun Any.matchesIcon(icon: IIcon) = (Refs.CPmatchTileIcons.get(this) as Array<IIcon>?)?.let { icon in it } ?: false
