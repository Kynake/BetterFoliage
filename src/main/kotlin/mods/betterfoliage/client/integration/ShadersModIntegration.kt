...
object ShadersModIntegration {
    ...
    fun entityDataFor(block: Block) = (Block.blockRegistry.getIDForObject(block) and 65535) or ((block.renderType and 65535) shl 16)
    ...
    init {
        if (allAvailable(Refs.pushEntity_I, Refs.popEntity)) {
            Client.log(Level.INFO, "ShadersMod integration enabled")
            isPresent = true
        }
    }
    ...
    /** Quads rendered inside this block will use the given block entity data in shader programs. */
    inline fun renderAs(block: Block, enabled: Boolean = true, func: () -> Unit) = renderAs(entityDataFor(block), enabled, func)
