...
object ShadersModIntegration {
  ...
  fun entityDataFor(block: Block) =
      (Block.blockRegistry.getIDForObject(block) and 65535) or ((block.renderType and 65535) shl 16)
  ...
  inline fun grass(enabled: Boolean = true, func: () -> Unit) =
      renderAs(tallGrassEntityData, enabled, func)

  /** Quads rendered inside this block will behave as leaf blocks in shader programs. */
  inline fun leaves(enabled: Boolean = true, func: () -> Unit) =
      renderAs(leavesEntityData, enabled, func)
}
