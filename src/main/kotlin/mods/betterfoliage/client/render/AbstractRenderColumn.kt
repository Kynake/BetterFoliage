...
abstract class AbstractRenderColumn(modId: String) : AbstractBlockRenderingHandler(modId) {
  ...
  inline fun extendTop(type: QuadrantType) =
      when (type) {
  ...
  inline fun extendBottom(type: QuadrantType) =
      when (type) {
  ...
  inline fun flatTop(type: QuadrantType) =
      when (type) {
  ...
  inline fun flatBottom(type: QuadrantType) =
      when (type) {
  ...
  val sideTexture: QuadIconResolver = { ctx, qi, _ ->
    if ((qi and 1) == 0) ctx.icon(SOUTH) else ctx.icon(EAST)
  }
    ...
    ShadersModIntegration.renderAs(ctx.block) {
      quadrantRotations.forEachIndexed { idx, quadrantRotation ->
        ...
        if (quadrants[idx] == LARGE_RADIUS &&
            upType == PARALLEL &&
            quadrantsTop[idx] != LARGE_RADIUS &&
            downType == PARALLEL &&
            quadrantsBottom[idx] != LARGE_RADIUS) {
        ...
        val sideModel =
            when (quadrants[idx]) {
              ...
              LARGE_RADIUS ->
                  if (upType == PARALLEL && quadrantsTop[idx] == SMALL_RADIUS) transitionTop.model
                  else if (downType == PARALLEL && quadrantsBottom[idx] == SMALL_RADIUS)
