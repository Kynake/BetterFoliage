...
class EntityFallingLeavesFX(world: World, x: Int, y: Int, z: Int) :
  ...
  override fun render(tessellator: Tessellator, partialTickTime: Float) {
    ...
    renderParticleQuad(
        tessellator, partialTickTime, rotation = particleRot, isMirrored = isMirrored)
  }
  ...
  fun changeWind(world: World) {
    ...
    val speed =
        abs(random.nextGaussian()) * Config.fallingLeaves.windStrength +
            (if (!world.isRaining) 0.0
            else abs(random.nextGaussian()) * Config.fallingLeaves.stormStrength)
