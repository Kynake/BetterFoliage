...
class EntityRisingSoulFX(world: World, x: Int, y: Int, z: Int) : AbstractEntityFX(world, x.toDouble() + 0.5, y.toDouble() + 1.0, z.toDouble() + 0.5) {
    ...
    init {
        ...
        particleMaxAge =
            MathHelper.floor_double((0.6 + 0.4 * rand.nextDouble()) * Config.risingSoul.lifetime * 20.0)
    }
    ...
    override fun render(tessellator: Tessellator, partialTickTime: Float) {
        ...
        renderParticleQuad(tessellator, partialTickTime, size = Config.risingSoul.headSize * 0.25, alpha = alpha)
...
object RisingSoulTextures : ResourceHandler(BetterFoliageMod.MOD_ID) {
    ...
    override fun afterStitch() {
        Client.log(Level.INFO, "Registered ${headIcons.num} soul particle textures")
    }
}
