...
import mods.octarinecore.client.render.cornerInterpolate
import mods.octarinecore.client.render.faceOrientedAuto
import mods.octarinecore.client.render.faceOrientedInterpolate
...
const val CHAMFER_AFFINITY = 0.9f

/** Amount to shrink column extension bits to stop Z-fighting. */
val zProtectionScale: Double3
    get() = Double3(Config.roundLogs.zProtection, 1.0, Config.roundLogs.zProtection)

fun Model.columnSide(
    radius: Double,
    yBottom: Double,
    yTop: Double,
    transform: (Quad) -> Quad = { it },
) {
    ...
    listOf(
        verticalRectangle(x1 = 0.0, z1 = 0.5, x2 = 0.5 - radius, z2 = 0.5, yBottom = yBottom, yTop = yTop)
            ...
            .setAoShader(faceOrientedInterpolate(overrideFace = ForgeDirection.SOUTH))
            .setAoShader(
                faceOrientedAuto(corner = cornerAo(Axis.Y)),
        ...
        verticalRectangle(
            ...
            .setAoShader(
                faceOrientedAuto(
                    overrideFace = ForgeDirection.SOUTH,
                    corner =
                    cornerInterpolate(Axis.Y, CHAMFER_AFFINITY, Config.roundLogs.dimming),
                ),
            )
            .setAoShader(
                faceOrientedAuto(
        ...
        .forEach { transform(it.setFlatShader(FaceFlat(ForgeDirection.SOUTH))).add() }
    ...
    listOf(
        verticalRectangle(
            ...
            .setAoShader(
                faceOrientedAuto(
                    overrideFace = ForgeDirection.EAST,
                    corner =
                    cornerInterpolate(Axis.Y, CHAMFER_AFFINITY, Config.roundLogs.dimming),
                ),
            )
        ...
        verticalRectangle(x1 = 0.5, z1 = 0.5 - radius, x2 = 0.5, z2 = 0.0, yBottom = yBottom, yTop = yTop)
            ...
            .setAoShader(faceOrientedInterpolate(overrideFace = ForgeDirection.EAST))
            .setAoShader(
                faceOrientedAuto(corner = cornerAo(Axis.Y)),
                predicate = { v, vi -> vi == 0 || vi == 3 },
            ),
    )
        .forEach { transform(it.setFlatShader(FaceFlat(ForgeDirection.EAST))).add() }
...
fun Model.columnSideSquare(yBottom: Double, yTop: Double, transform: (Quad) -> Quad = { it }) {
    listOf(
        ...
        .forEach { transform(it.setFlatShader(faceOrientedAuto(corner = cornerFlat))).add() }
}
...
fun Model.columnLid(radius: Double, transform: (Quad) -> Quad = { it }) {
    ...
    val q1 =
        Quad(v1, v2, v3, v4)
            .setAoShader(faceOrientedAuto(overrideFace = ForgeDirection.UP, corner = cornerAo(Axis.Y)))
            .transformVI { vertex, idx ->
                vertex.copy(
                    aoShader =
                    when (idx) {
                        0 -> FaceCenter(ForgeDirection.UP)
                        1 -> EdgeInterpolateFallback(ForgeDirection.UP, ForgeDirection.SOUTH, 0.0)
        ...
        Quad(v1, v4, v5, v6)
            ...
            .transformVI { vertex, idx ->
                vertex.copy(
                    ...
                    when (idx) {
                        0 -> FaceCenter(ForgeDirection.UP)
                        3 -> EdgeInterpolateFallback(ForgeDirection.UP, ForgeDirection.EAST, 0.0)
                        else -> vertex.aoShader
                    },
                )
            }
    listOf(q1, q2).forEach { transform(it.setFlatShader(FaceFlat(ForgeDirection.UP))).add() }
}
...
fun Model.columnLidSquare(transform: (Quad) -> Quad = { it }) {
    transform(
        horizontalRectangle(x1 = 0.0, x2 = 0.5, z1 = 0.0, z2 = 0.5, y = 0.5)
            .transformVI { vertex, idx ->
                vertex.copy(
                    uv = UV(vertex.xyz.x, vertex.xyz.z),
                    aoShader =
                    when (idx) {
                        0 -> FaceCenter(ForgeDirection.UP)
                        1 -> EdgeInterpolateFallback(ForgeDirection.UP, ForgeDirection.SOUTH, 0.0)
                        2 -> CornerSingleFallback(ForgeDirection.UP, ForgeDirection.SOUTH, ForgeDirection.EAST, ForgeDirection.UP)
                        else -> EdgeInterpolateFallback(ForgeDirection.UP, ForgeDirection.EAST, 0.0)
                    },
                )
            }
            .setFlatShader(FaceFlat(ForgeDirection.UP)),
    )
        .add()
}
