...
import mods.octarinecore.client.render.forgeDirOffsets
...
class RenderCoral : AbstractBlockRenderingHandler(BetterFoliageMod.MOD_ID) {
    ...
    val coralModels =
        modelSet(64) { modelIdx ->
            verticalRectangle(x1 = -0.5, z1 = 0.5, x2 = 0.5, z2 = -0.5, yBottom = 0.0, yTop = 1.0)
                .scale(Config.coral.size)
                .move(0.5 to UP)
                .toCross(UP) { it.move(xzDisk(modelIdx) * Config.coral.hOffset) }
                .addAll()
    ...
    override fun render(ctx: BlockContext, parent: RenderBlocks): Boolean {
        ...
        forgeDirs.forEachIndexed { idx, face ->
            if (!ctx.block(forgeDirOffsets[idx]).isOpaqueCube &&
                blockContext.random(idx) < Config.coral.chance
            ) {
                ...
                modelRenderer.render(
                    ...
                    rotationFromUp[idx],
                    icon = { _, qi, _ ->
                        if (qi == 4) crustIcons[variation]!! else coralIcons[variation + (qi and 1)]!!
                    },
                    rotateUV = { 0 },
                    postProcess = noPost,
                )
