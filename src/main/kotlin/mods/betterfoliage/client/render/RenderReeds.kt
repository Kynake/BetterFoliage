...
class RenderReeds : AbstractBlockRenderingHandler(BetterFoliageMod.MOD_ID) {
    ...
    val reedIcons =
        iconSet(Client.genReeds.generatedResource("${BetterFoliageMod.LEGACY_DOMAIN}:better_reed_%d"))
    val reedModels =
        modelSet(64) { modelIdx ->
            ...
            listOf(
                ...
                verticalRectangle(
                    ...
                    .setFlatShader(FlatOffsetNoColor(up1))
                    .clampUV(minV = vCutLine),
                ...
                verticalRectangle(
                    ...
                    x2 = 0.5,
                    z2 = -0.5,
                    yBottom = 0.5 + waterline,
                    yTop = 0.5 + height,
                )
                    .setFlatShader(FlatOffsetNoColor(up2))
                    .clampUV(maxV = vCutLine),
            )
                .forEach {
                    it.clampUV(minU = -0.25, maxU = 0.25)
                        .toCross(UP) { it.move(xzDisk(modelIdx) * Config.reed.hOffset) }
                        .addAll()
                }
        }
