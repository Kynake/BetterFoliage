...
/**
 * Generate round leaf textures from leaf block textures. The base texture is tiled 2x2, then parts
 * of it are made transparent by applying a mask to the alpha channel.
 *
 * Generator parameter _type_: Leaf type (configurable by user). Different leaf types may have their
 * own alpha mask.
 *
...
class LeafGenerator(domain: String) : TextureGenerator(domain) {
  ...
  override fun generate(params: ParameterList): BufferedImage? {
    ...
    val handDrawnLoc =
        target.second.stripStart("textures/").stripStart("blocks/").let {
          ResourceLocation(
              BetterFoliageMod.DOMAIN, "${it.resourceDomain}/textures/blocks/${it.resourcePath}")
        }
    resourceManager[handDrawnLoc]?.loadImage()?.let {
      return it
    }
    ...
    val maskTexture =
        (getLeafMask(leafType, size * 2) ?: getLeafMask("default", size * 2))?.loadImage()
   ...
   */
  fun getLeafMask(type: String, maxSize: Int) =
      getMultisizeTexture(maxSize) { size ->
