...
class TextureMatcher() {
  ...
  fun getType(icon: TextureAtlasSprite): String? =
      mappings.filter { it.matches(icon) }.map { it.type }.firstOrNull()
          ...
          .forEach { line ->
            ...
            if (line2.size == 2) {
              ...
              if (mapping.size == 1) mappings.add(Mapping(null, mapping[0].trim(), line2[1].trim()))
                  ...
                  mappings.add(Mapping(mapping[0].trim(), mapping[1].trim(), line2[1].trim()))
            }
          }
    }
  }
