...
abstract class AbstractBlockRenderingHandler(modId: String) :
    ResourceHandler(modId), ISimpleBlockRenderingHandler {
  ...
  fun renderWorldBlockBase(
    ...
    if (handler != null && ctx.block.renderType != 0) {
      handler.renderWorldBlock(
          ctx.world, ctx.x, ctx.y, ctx.z, ctx.block, ctx.block.renderType, renderBlocks)
    } else {
      block()
    }
    return false
...
class BlockContext() {
  ...
  fun blockColor(offset: Int3) =
      block(offset).colorMultiplier(world, x + offset.x, y + offset.y, z + offset.z)
  ...
  fun icon(face: ForgeDirection, offset: Int3 = Int3.zero) =
      block(offset).getIcon(world, x + offset.x, y + offset.y, z + offset.z, face.ordinal).let {
        if (!OptifineCTM.isAvailable) it
            ...
            Refs.getConnectedTexture.invokeStatic(
                world!!, block(offset), x + offset.x, y + offset.y, z + offset.z, face.ordinal, it)
                as IIcon
      }
