...
val Pair<Axis, Dir>.face: ForgeDirection
    get() =
        when (this) {
            Axis.X to P -> ForgeDirection.EAST
            Axis.X to N -> ForgeDirection.WEST
            Axis.Y to P -> ForgeDirection.UP
            Axis.Y to N -> ForgeDirection.DOWN
            Axis.Z to P -> ForgeDirection.SOUTH
            Axis.Z to N -> ForgeDirection.NORTH
            else -> ForgeDirection.UNKNOWN
        }
val ForgeDirection.perpendiculars: List<ForgeDirection>
    get() = axes.filter { it != this.axis }.cross(axisDirs).map { it.face }
val ForgeDirection.offset: Int3
    get() = forgeDirOffsets[ordinal]

// ================================
// Vectors
// ================================
operator fun ForgeDirection.times(scale: Double) = Double3(offsetX.toDouble() * scale, offsetY.toDouble() * scale, offsetZ.toDouble() * scale)

val ForgeDirection.vec: Double3
    get() = Double3(offsetX.toDouble(), offsetY.toDouble(), offsetZ.toDouble())

/**
 * 3D vector of [Double]s. Offers both mutable operations, and immutable operations in operator
 * notation.
 */
data class Double3(var x: Double, var y: Double, var z: Double) {
    ...
    fun rotate(rot: Rotation) = Double3(
        rot.rotatedComponent(ForgeDirection.EAST, x, y, z),
        rot.rotatedComponent(ForgeDirection.UP, x, y, z),
        rot.rotatedComponent(ForgeDirection.SOUTH, x, y, z),
    )
    ...
    val normalize: Double3
        get() = (1.0 / length).let { Double3(x * it, y * it, z * it) }
    val nearestCardinal: ForgeDirection
        get() = nearestAngle(this, forgeDirs.asIterable()) { it.vec }.first
}

/**
 * 3D vector of [Int]s. Offers both mutable operations, and immutable operations in operator
 * notation.
 */
...
/**
 * Class representing an arbitrary rotation (or combination of rotations) around cardinal axes by 90
 * degrees. In effect, a permutation of [ForgeDirection]s.
 */
...
class Rotation(val forward: Array<ForgeDirection>, val reverse: Array<ForgeDirection>) {
    ...
    inline fun rotatedComponent(dir: ForgeDirection, x: Int, y: Int, z: Int) = when (reverse[dir.ordinal]) {
        ForgeDirection.EAST -> x
        ForgeDirection.WEST -> -x
        ForgeDirection.UP -> y
        ForgeDirection.DOWN -> -y
        ForgeDirection.SOUTH -> z
        ForgeDirection.NORTH -> -z
        else -> 0
    }
    inline fun rotatedComponent(dir: ForgeDirection, x: Double, y: Double, z: Double) = when (reverse[dir.ordinal]) {
        ForgeDirection.EAST -> x
        ForgeDirection.WEST -> -x
        ForgeDirection.UP -> y
        ForgeDirection.DOWN -> -y
    ...
    companion object {
        ...
        val rot90 =
            Array(6) { idx -> Rotation(forgeDirs[idx].opposite.rotations, forgeDirs[idx].rotations) }
...
val faceCorners =
    forgeDirs.map {
        when (it) {
            ForgeDirection.DOWN -> FaceCorners(ForgeDirection.SOUTH, ForgeDirection.WEST)
            ForgeDirection.UP -> FaceCorners(ForgeDirection.SOUTH, ForgeDirection.EAST)
            ForgeDirection.NORTH -> FaceCorners(ForgeDirection.WEST, ForgeDirection.UP)
            ForgeDirection.SOUTH -> FaceCorners(ForgeDirection.UP, ForgeDirection.WEST)
            ForgeDirection.WEST -> FaceCorners(ForgeDirection.SOUTH, ForgeDirection.UP)
            ForgeDirection.EAST -> FaceCorners(ForgeDirection.SOUTH, ForgeDirection.DOWN)
