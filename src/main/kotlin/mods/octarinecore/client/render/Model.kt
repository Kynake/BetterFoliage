...
data class Quad(val v1: Vertex, val v2: Vertex, val v3: Vertex, val v4: Vertex) {
    ...
    inline fun transformV(trans: (Vertex) -> Vertex): Quad = transformVI { vertex, idx ->
        trans(vertex)
    }
    inline fun transformVI(trans: (Vertex, Int) -> Vertex): Quad = Quad(trans(v1, 0), trans(v2, 1), trans(v3, 2), trans(v4, 3))
    val normal: Double3
    ...
    fun rotate(rot: Rotation) = transformV {
        it.copy(
            ...
            aoShader = it.aoShader.rotate(rot),
    ...
    fun setFlatShader(
        ...
        predicate: (Vertex, Int) -> Boolean = { v, vi -> true },
