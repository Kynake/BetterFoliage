...
class ModelRenderer : ShadingContext() {
    ...
    /**
     * Render a [Model]. The [blockContext] and [renderBlocks] need to be set up correctly, including
     * first rendering the corresponding block to capture shading values!
     *
     ...
     * @param[postProcess] lambda to perform arbitrary modifications on the [RenderVertex] just before
     * it goes to the [Tessellator]
     */
    fun render(
        model: Model,
...
/**
 * Queried by [Shader] objects to get rendering-relevant data of the current block in a rotated
 * frame of reference.
 */
open class ShadingContext {
    ...
    fun aoShading(face: ForgeDirection, corner1: ForgeDirection, corner2: ForgeDirection) = renderBlocks.capture.aoShading(face.rotate(rotation), corner1.rotate(rotation), corner2.rotate(rotation))
