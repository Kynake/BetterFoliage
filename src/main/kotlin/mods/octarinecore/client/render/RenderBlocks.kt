...
val renderingHandlers: Map<Int, ISimpleBlockRenderingHandler> =
...
class ExtendedRenderBlocks : RenderBlocks() {
    ...
    override fun renderFaceXPos(block: Block?, x: Double, y: Double, z: Double, icon: IIcon?) = renderFace(ForgeDirection.EAST, block, x, y, z, icon)
    override fun renderFaceXNeg(block: Block?, x: Double, y: Double, z: Double, icon: IIcon?) = renderFace(ForgeDirection.WEST, block, x, y, z, icon)
    override fun renderFaceYPos(block: Block?, x: Double, y: Double, z: Double, icon: IIcon?) = renderFace(ForgeDirection.UP, block, x, y, z, icon)
    override fun renderFaceYNeg(block: Block?, x: Double, y: Double, z: Double, icon: IIcon?) = renderFace(ForgeDirection.DOWN, block, x, y, z, icon)
    override fun renderFaceZPos(block: Block?, x: Double, y: Double, z: Double, icon: IIcon?) = renderFace(ForgeDirection.SOUTH, block, x, y, z, icon)
    override fun renderFaceZNeg(block: Block?, x: Double, y: Double, z: Double, icon: IIcon?) = renderFace(ForgeDirection.NORTH, block, x, y, z, icon)
    ...
    fun renderFace(
        ...
        if (capture.renderCallback(capture, face, capture.passes[face.ordinal], icon)) {
            when (face) {
                ForgeDirection.EAST -> super.renderFaceXPos(block, x, y, z, icon)
                ForgeDirection.WEST -> super.renderFaceXNeg(block, x, y, z, icon)
                ForgeDirection.UP -> super.renderFaceYPos(block, x, y, z, icon)
                ForgeDirection.DOWN -> super.renderFaceYNeg(block, x, y, z, icon)
                ForgeDirection.SOUTH -> super.renderFaceZPos(block, x, y, z, icon)
                ForgeDirection.NORTH -> super.renderFaceZNeg(block, x, y, z, icon)
                else -> {}
            }
        }
    }

    fun saveTopLeft(face: ForgeDirection, corner: Pair<ForgeDirection, ForgeDirection>) = capture
        .aoShading(face, corner.first, corner.second)
        .set(brightnessTopLeft, colorRedTopLeft, colorGreenTopLeft, colorBlueTopLeft)

    fun saveTopRight(face: ForgeDirection, corner: Pair<ForgeDirection, ForgeDirection>) = capture
        .aoShading(face, corner.first, corner.second)
        .set(brightnessTopRight, colorRedTopRight, colorGreenTopRight, colorBlueTopRight)

    fun saveBottomLeft(face: ForgeDirection, corner: Pair<ForgeDirection, ForgeDirection>) = capture
        .aoShading(face, corner.first, corner.second)
        .set(brightnessBottomLeft, colorRedBottomLeft, colorGreenBottomLeft, colorBlueBottomLeft)

    fun saveBottomRight(face: ForgeDirection, corner: Pair<ForgeDirection, ForgeDirection>) = capture
        .aoShading(face, corner.first, corner.second)
        .set(
...
class ShadingCapture {
    ...
    init {
        (0..5).forEach { i1 ->
            (0..5).forEach { i2 -> (i2..5).forEach { i3 -> aoShadings[cornerId(i1, i2, i3)] = AoData() } }
        }
     ...
     */
    fun aoShading(face: ForgeDirection, corner1: ForgeDirection, corner2: ForgeDirection) = aoShadings[cornerId(face, corner1, corner2)]!!
