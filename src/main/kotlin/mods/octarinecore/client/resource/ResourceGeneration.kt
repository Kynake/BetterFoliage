...
/**
 * [IResourcePack] containing generated resources. Adds itself to the default resource pack list of
 * Minecraft, so it is invisible and always active.
 *
...
class GeneratorPack(val name: String, vararg val generators: GeneratorBase) : IResourcePack {
    ...
    override fun getPackMetadata(serializer: IMetadataSerializer?, type: String?) = if (type == "pack") PackMetadataSection(ChatComponentText("Generated resources"), 1) else null

    override fun resourceExists(location: ResourceLocation?): Boolean = if (location == null) {
        false
    } else {
        generators.find { it.domain == location.resourceDomain && it.resourceExists(location) } !=
            null
    }

    override fun getInputStream(location: ResourceLocation?): InputStream? = if (location == null) {
        null
    } else {
        generators
...
/**
 * Collection of named [String]-valued key-value pairs, with an extra unnamed (keyless) value. Meant
 * to be encoded as a pipe-delimited list, and used as a [ResourceLocation] path to parametrized
 * generated resources.
 *
...
class ParameterList(val params: Map<String, String>, val value: String?) {
    override fun toString() = params.entries
        .sortedBy { it.key }
...
abstract class ParameterBasedGenerator(domain: String) : GeneratorBase(domain) {
    ...
    override fun resourceExists(location: ResourceLocation?) = resourceExists(ParameterList.fromString(location?.resourcePath ?: ""))
    override fun getInputStream(location: ResourceLocation?) = getInputStream(ParameterList.fromString(location?.resourcePath ?: ""))
}
