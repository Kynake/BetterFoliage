...
abstract class TextureGenerator(domain: String) : ParameterBasedGenerator(domain) {
     ...
     *
     * @param[iconName] the name of the [TextureAtlasSprite] (not the full location) backing the
     * generated texture
    ...
    fun targetResource(params: ParameterList): Pair<ResourceType, ResourceLocation>? {
        ...
        return when (params.value?.lowercase()) {
            "generate.png" -> ResourceType.COLOR to baseTexture + ".png"
            "generate.png.mcmeta" -> ResourceType.METADATA to baseTexture + ".png.mcmeta"
            "generate_n.png" -> ResourceType.NORMAL to baseTexture + "_n.png"
            "generate_s.png" -> ResourceType.SPECULAR to baseTexture + "_s.png"
            else -> null
        }
    }

    override fun resourceExists(params: ParameterList) = targetResource(params)?.second?.let { resourceManager[it] != null } ?: false

    override fun getInputStream(params: ParameterList): InputStream? {
        val target = targetResource(params)
        return when (target?.first) {
            null -> null
            ResourceType.METADATA -> resourceManager[target!!.second]?.inputStream
     ...
     *
     * @param[maxSize] Maximum size to consider. This value is progressively halved when searching for
     * smaller versions.
