...
operator fun ResourceLocation.plus(str: String) =
    ResourceLocation(resourceDomain, resourcePath + str)

/** Index operator to get a resource. */
operator fun IResourceManager.get(domain: String, path: String): IResource? =
    get(ResourceLocation(domain, path))
/** Index operator to get a resource. */
operator fun IResourceManager.get(location: ResourceLocation): IResource? =
    tryDefault(null) { getResource(location) }
 ...
 *
 * Only non-transparent pixels are considered. Averages are taken in the HSB color space (note: Hue
 * is a circular average), and the result transformed back to the RGB color space.
 */
val TextureAtlasSprite.averageColor: Int?
  get() {
    val locationNoDirs = ResourceLocation(iconName)
    val locationWithDirs =
        ResourceLocation(
    ...
    for (x in 0..image.width - 1) for (y in 0..image.height - 1) {
      val pixel = image[x, y]
...
fun textureLocation(iconName: String) =
    ResourceLocation(iconName).let {
