 ...
 *
 * Subclasses should be singleton objects, containing one layer of further singleton objects
 * representing config categories (nesting is not supported).
 *
 * Both the root object (maps to the category _global_) and category objects can contain
 * [ConfigPropertyBase] instances (either directly or as a delegate), which handle the Forge
...
abstract class DelegatingConfig(val modId: String, val langPrefix: String) {
    ...
    fun attach(config: Configuration) {
        ...
        forEachProperty { category, name, property ->
            ...
            property.guiProperties.forEach { guiProperty ->
                ...
                if (category == "global") {
                    rootGuiElements.add(ConfigElement.getTypedElement(guiProperty))
                } else {
                    subProperties.put(category, guiProperty.name)
                }
            }
        }
    ...
    /**
     * Execute the given lambda for all config properties. Lambda params: (category name, property
    ...
    /**
     * Returns true if any of the given configuration elements have changed. Supports both categories
     * and
     */
    fun hasChanged(vararg elements: Any?): Boolean {
        reflectNestedObjects.forEach { category ->
            if (category.second in elements && config?.getCategory(category.first)?.hasChanged() ?: false) {
                return true
            }
        }
        forEachProperty { category, name, property ->
...
abstract class ConfigPropertyDelegate<T> : ConfigPropertyBase() {
    ...
    override val guiProperties: List<Property>
        get() = listOf(property!!)
    override val hasChanged: Boolean
        get() = property?.hasChanged() ?: false
...
class ConfigPropertyDouble(val min: Double, val max: Double, val default: Double) : ConfigPropertyDelegate<Double>() {
    override fun resolve(target: Configuration, category: String, name: String) = target.get(category, name, default, null).apply {
        setMinValue(min)
        setMaxValue(max)
    }
...
class ConfigPropertyFloat(val min: Double, val max: Double, val default: Double) : ConfigPropertyDelegate<Float>() {
    override fun resolve(target: Configuration, category: String, name: String) = target.get(category, name, default, null).apply {
        setMinValue(min)
        setMaxValue(max)
    }
...
class ConfigPropertyInt(val min: Int, val max: Int, val default: Int) : ConfigPropertyDelegate<Int>() {
    override fun resolve(target: Configuration, category: String, name: String) = target.get(category, name, default, null).apply {
        setMinValue(min)
        setMaxValue(max)
    }
...
class ConfigPropertyBoolean(val default: Boolean) : ConfigPropertyDelegate<Boolean>() {
    override fun resolve(target: Configuration, category: String, name: String) = target.get(category, name, default, null)
...
class ConfigPropertyIntList(val defaults: () -> Array<Int>) : ConfigPropertyDelegate<Array<Int>>() {
    override fun resolve(target: Configuration, category: String, name: String) = target.get(category, name, defaults().toIntArray(), null)
