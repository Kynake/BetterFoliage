...
inline fun <reified T> Any.reflectField(field: String): T? =
    tryDefault(null) { this.javaClass.getDeclaredField(field) }
        ?.let {
...
inline fun <reified T> Class<*>.reflectStaticField(field: String): T? =
    tryDefault(null) { this.getDeclaredField(field) }
        ?.let {
          it.isAccessible = true
          it.get(null) as T
        }
 ...
 */
fun Any.reflectFieldsOfType(vararg types: Class<*>) =
...
open class ClassRef(val mcpName: String, val obfName: String) : Resolvable<Class<*>>() {
  ...
  override fun resolve() =
      listOf(mcpName, obfName).map { getJavaClass(it) }.filterNotNull().firstOrNull()
}
...
class MethodRef(
  ...
  fun asmDescriptor(namespace: Namespace) =
      "(${argTypes.map { it.asmDescriptor(namespace) }.fold(""){ s1, s2 -> s1 + s2 } })${returnType.asmDescriptor(namespace)}"
...
class FieldRef(
  ...
  constructor(
      ...
      mcpName: String,
      type: ClassRef?
  ) : this(parentClass, mcpName, mcpName, mcpName, type)
