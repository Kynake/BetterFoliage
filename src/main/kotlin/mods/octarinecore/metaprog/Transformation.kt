...
open class Transformer : IClassTransformer {
   ...
   */
  fun transformMethod(method: MethodRef, trans: MethodTransformContext.() -> Unit) =
      transformers.add(method to trans)
  ...
  override fun transform(
    ...
    val transformations: List<Pair<MethodTransformContext.() -> Unit, MethodNode?>> =
        transformers.map { transformer ->
          ...
          log.debug(
              "  searching: ${transformer.first.name(OBF)} ${transformer.first.asmDescriptor(OBF)} -> ${transformer.first.name(MCP)} ${transformer.first.asmDescriptor(MCP)}")
          transformer.second to
              classNode.methods.find {
                log.debug("             ${it.name} ${it.desc}")

                it.name == transformer.first.name(MCP) &&
                    it.desc == transformer.first.asmDescriptor(MCP) ||
                    it.name == transformer.first.name(OBF) &&
                        it.desc == transformer.first.asmDescriptor(OBF)
              }
        }
    ...
    return if (!workDone) classData
    else ClassWriter(0).apply { classNode.accept(this) }.toByteArray()
  }
}
...
class MethodTransformContext(val method: MethodNode, val environment: Namespace) {
  ...
  fun find(predicate: (AbstractInsnNode) -> Boolean): AbstractInsnNode? =
      find(method.instructions.first, predicate)
   ...
   */
  fun AbstractInsnNode.insertAfter(init: InstructionList.() -> Unit) =
      InstructionList(environment).apply {
        this.init()
   ...
   * @param[init] builder-style lambda to assemble instruction list
   */
  fun AbstractInsnNode.insertBefore(init: InstructionList.() -> Unit) =
      InstructionList(environment).apply {
        this.init()
...
class InstructionList(val environment: Namespace) {
   ...
   */
  fun invokeStatic(target: MethodRef, isInterface: Boolean = false) =
      list.add(
          MethodInsnNode(
